...
test("forEach", () => {
  ...
  const result = hwp.forEach(generator(), async (item) => {
    // This is just a type test, we're not actually calling console.log
    item.toFixed();
  });
...
test("mapper", () => {
  ...
  const isFunction: (iterator: AsyncIterable<number>) => AsyncGenerator<string, void, unknown> = mapperFn;
